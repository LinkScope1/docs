...
统计与报表、批量导入、异步任务以及 Casdoor、LinkForty、NFC 集成属于横向或外部能力，不占用 M 编号。统计与报表只读 M5 的 `access_events` 和 LinkForty 授权事件数据，不拥有本地专属表。
...
| P2 | 工作包 A 闭环 | M1、M2、M4 完成审计、主数据和绑定生命周期 |
| P3 | 工作包 B 闭环 | M3、M5 完成 NFC 资产内容、LinkForty 集成和事件关联 |
