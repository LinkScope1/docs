...
模块编号规则：新需求、Issue、分支、Commit、PR、代码注释和验收项只能使用 M1～M5。旧 M1～M8 仅可在明确标注“历史编号迁移”的章节、版本日志或数据字典修订记录中出现；不得把旧 M7 访问事件或旧 M8 统计分析当作当前模块。Webhook 接收、验签、幂等投影和事件关联统一归属 M5；统计与导出继续作为无编号横向能力。

提交前应使用 `rg -n '\bM[6-8]\b'` 检查变更范围。每个命中都必须确认属于历史迁移说明；活动任务、当前模块表、验收项、PR 正文和代码归属中不得出现 M6～M8。
